...
def az():
    return AzProductInfo(None, None, 'tests/tests.json')
...
@pytest.mark.parametrize("service, cloud, expected_result", [
    ...
    ('Full Service', 'azure-public', True),
    ('Full Service', 'azure-government', True)
])
...
@pytest.mark.parametrize("prod, region, expected_result", [
    ...
    ('Full Service', 'gcc-ga-reg-1', True)
])
...
@pytest.mark.parametrize("product, cloud", [
    ...
    #    ('Microsoft Genomics', False),
    ('Full Service', 'azure-public'),
    ('Full Service', 'azure-government')
])
...
@pytest.mark.parametrize("product, expected_result", [
    ...
    #    ('Anomoly Detector', 'usgov-virginia'),
    #    ('Microsoft Genomics', False),
    ('Full Service', 'gcc-prev-reg-1'),
    ('Full Service', 'mag-prev-reg-1')
])
...
@pytest.mark.parametrize("product, scope, cloud, expected_result", [
    ('Full Service', 'gcc-scope-2', 'azure-public', True),
    ('Full Service', 'gcc-scope-3', 'azure-public', True),
    ('Full Service', 'gcc-scope-4', 'azure-public', False),
    ('Full Service', 'mag-scope-1', 'azure-government', True),
    ('Full Service', 'mag-scope-2', 'azure-government', True),
    ('Full Service', 'mag-scope-3', 'azure-government', False),

])
def test_isAtAuditScope(az, product, scope, cloud, expected_result):
    assert az.isAtAuditScope(product, scope, cloud) == expected_result
...
@pytest.mark.parametrize("product, region, target", [
    ('Full Service', 'gcc-prev-reg-1', 'Q2 2021'),
    ('Full Service', 'mag-prev-reg-1', 'Q2 2021'),
    ('Not GA Service', 'gcc-prev-reg-1', 'Q2 2021'),
    ('Not GA Service', 'mag-prev-reg-1', 'Q2 2021'),
])
def test_is_ga_expected(az, product, region, target):
    tmp = {
        'region': region,
        'ga-expected': target
    }

    ga_expected = az.getProductRegionsGATargets(product)
    assert tmp in ga_expected
