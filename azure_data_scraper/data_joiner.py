...
class DataJoiner:
    def __init__(self, audit_scopes=AuditScopes(), products_by_region=ProductsByRegion()):
        self.__product_list = self.__hydrate_product_list(audit_scopes.getAuditScopeDictionary(), products_by_region.getProductsAvailabilityDictionary())
        self.__category_list = self.__hydrate_category_list()

        #print (json.dumps(self.__product_list))

        #pass
    

    def __hydrate_product_list(self, audit_scopes, products_by_region) -> dict:
        
        product_list = products_by_region.copy()
        
        for i, prod in product_list.items():
            ...
            product_list[i].pop('doc-type')
        
        return product_list

    def __hydrate_category_list(self):
        cat_set = set()

        for prod in self.__product_list.values():            
        ...
        for id, prod in self.__product_list.items():            
        ...
        for id in (maps.service_list + maps.capability_list):
            for scope in (maps.us_scopes + maps.usgov_scopes):
                if not scope.__contains__("Il5"):
                    print("Is %s at %s? \t" % (
                        id, maps.scope_map[scope]), self.answer_isAtScope(id, scope))
        ...
        for id in (maps.service_list + maps.capability_list):
            for region in (maps.us_regions + maps.usgov_regions + maps.usdod_regions):
                if not region.__contains__("non-regional"):
                    print("Is %s in %s? \t" % (id, region),
                          self.answer_isInRegion(id, region))
